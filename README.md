...
> **TopBid** is the brand; the repo, install script, and `~/.kickbacks` paths keep the `kickbacks` name so existing installs and the public install URL keep working.
...
The installer writes the live Worker endpoint to `~/.kickbacks/endpoint`, and the renderer calls `/ad` every 30 seconds. Links submitted into the Worker-backed list start appearing in terminals automatically after the local cache refreshes. `KICKBACKS_ENDPOINT` can still override the endpoint for testing.

## What it touches (and what it doesn't)

It writes one script to `~/.kickbacks/` and adds a `statusLine` key to `~/.claude/settings.json`, backing up the old file first. It does not read your code, your prompts, your conversation, your environment variables, or the session data Claude Code hands the script. That input is dropped on the floor. The only thing that ever leaves your machine is a random key used to count ad views, and only once you set an endpoint. With no endpoint, it runs offline and nothing leaves.
...
Prefer to watch it happen? Copy `skills/kickbacks-setup/` into `~/.claude/skills/` and tell Claude Code "set up TopBid". It will audit the script in front of you before installing anything.
...
```
curl "$KICKBACKS_ENDPOINT/me?key=$(cat ~/.kickbacks/key)"
```
...
export KICKBACKS_ENDPOINT="https://topbid.bankingvaibhav.workers.dev"
```

Read the current live list:

```
curl "$KICKBACKS_ENDPOINT/admin/market?token=$ADMIN_TOKEN"
```

Add or replace one live link. If `id` already exists, it updates that row; otherwise it appends it:

```
curl -X POST "$KICKBACKS_ENDPOINT/admin/market?token=$ADMIN_TOKEN" \
...
```
curl -X POST "$KICKBACKS_ENDPOINT/admin/market/bulk?token=$ADMIN_TOKEN" \
  -H "content-type: application/json" \
...
```
curl -X POST "$KICKBACKS_ENDPOINT/admin/market/delete?id=some-link-id&token=$ADMIN_TOKEN"
```
...
```
rm -rf ~/.kickbacks
# then restore ~/.claude/settings.json from its .kickbacks.bak, or delete the statusLine key
```
